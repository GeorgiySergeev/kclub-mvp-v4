...
import { staffDeactivateSchema } from '@kclub/validation';
import { adminGuard } from '@/server/admin-guard';
import { jsonSuccess, jsonErrorFromUnknown } from '@/server/api';
import { deactivateStaff } from '@/server/services/admin-service';
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const result = await deactivateStaff(id, input, context);
